...
// GET /api/properties/admin — protected, list for the admin panel.
// Returns ALL properties (including drafts), unlike the public route. Trims
// images/brochureUrl the same way the public list does: an unfiltered
// find({}) can't use the { isPublished, order, createdAt } index (no
// isPublished equality to match its prefix), so without trimming this would
// reproduce the exact in-memory-sort memory crash we fixed earlier.
router.get('/admin', protect, async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
...
// GET /api/properties/admin/:id — protected, full single property (all
// images + brochureUrl) for the edit form. The list route above is trimmed,
// so this is what PropertyForm should fetch right before opening for edit —
// same pattern as the blog admin's fetch-full-post-before-editing flow.
router.get('/admin/:id', protect, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
